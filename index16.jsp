...
<script>
    ...
    nedenaciklama,etkilendigialanlar,kaynak,baslangictarihi,ilce,mahalle;
   function method3()
   {
   ...
   function ilcedegisimi() {
    ...
    function koydegisimi()
    {
 	    ...
 	    window.location.replace("index15.jsp?il="+il+"&afetturu="+afetturu+"&baslangictarihi="+bt+"&serino="+
 	    		serino+"&glideno="+glideno+"&bittar="+bittar+"&sure="+sure+"&belde="+belde+"&neden="+neden+
 	    		 "&enlem="+enlem+"&boylam="+boylam+"&nedenaciklama="+nedenaciklama+"&etkilendigialanlar="+
 	    		 etkilendigialanlar+"&kaynak="+kaynak+"&baslangictarihi="+baslangictarihi+"&ilce="+ilce+
 	    		 "&koy="+koy);
    }
    function mahalleyibul()
    {
    	il = document.getElementById("il").value;
 	    afetturu = document.getElementById("afetturu").value;
 	    bt = document.getElementById("bt").value;
...
<form action="ServletUygulama" method="post">
 <table id="firsttable">
        ...
        <td><select name="ilce" id="ilce" onchange="ilcedegisimi()">
          ...
          <%
            ...
            for(int i=0;i<ilceler.size();i++)
            {
            	...
            	if(k.getIlceAdi().equalsIgnoreCase(ilce))
            	{
            		continue;
            	}else{
     ...
     <tr>
        ...
        <td>Mahalle</td>
        <td><select id="mahalle" name="mahalle" onchange="mahalleyibul()">
          <option><% out.println(mahalle); %></option>
          <%
            MahalleDAOImpl mdoi = new MahalleDAOImpl();
           List<Mahalle> mahalleler = mdoi.koyeGoreMahalleyiBul(koy);
           for(int i=0;i<mahalleler.size();i++)
           {
        	   Mahalle mah = mahalleler.get(i);
        	       if(mah.getMahalleAdi().equalsIgnoreCase(mahalle))
        	       {
        	    	   continue;
        	       }else{
